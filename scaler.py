...
nodes_list = [("192.168.99.100:2376", scontext_node), ("192.168.99.101:2376", scontext_manager)]

# the address your managers nodes REST API is listening
manager = "192.168.99.101:2376"
...
def service_thread(arg):
    ...
    while True:
        ...
        # PID Controller
        print("PID Controller for service: {0}".format(service_in_analysis_name))
        ...
        output_integral_controller = integral_gain * sum(cpu_usage_avg_arr[-interval: ])
        ...
        if cpu_usage_avg > cpu_upper_threshold:
            ...
            task_count = len(services[service_in_analysis_name]["tasks"])
            if task_count > 1:
                print("Scalling down: {0}".format(task_count - scale_num))
                scale(services[service_in_analysis_name], task_count - scale_num)
        else:  # do nothing
            pass
...
thread_graphql_api = Thread(target = service_thread, args = ["home_controller_graphql-api"])
thread_graphql_api.start()

#thread_eeg_brainwave_api = Thread(target = service_thread, args = ["home_controller_eeg-brainwave-api"])
#thread_eeg_brainwave_api.start()
